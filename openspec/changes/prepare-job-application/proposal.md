...
- Run a **one-shot Mastra workflow** that (1) extracts/normalizes the job posting, (2) selects the user's best-matching base CV using posting content + user message, (3) **clones** that CV with a `source_cv_id` reference, (4) **tailors** the clone (update `basics.label`, bold Markdown in relevant summaries/highlights, trim irrelevant bullets from the copy), and (5) generates a **cover letter** as **Markdown**.
...
- Expose an **application workspace** in the web app: job summary, Markdown cover letter (copy as plain text for email, optional PDF export), and standard CV section editing on the clone. **No AI chat**—refinement is manual in the editor after generation.
