...
Require new accounts to confirm their email address before they can sign in, gated behind the `AUTH_EMAIL_VERIFICATION_ENABLED` feature flag. Verified accounts gate access to dashboard routes; unverified accounts are bounced to a "check your email" page.
...
1. `AUTH_EMAIL_VERIFICATION_ENABLED` in `apps/api/.env` (and `NEXT_PUBLIC_AUTH_EMAIL_VERIFICATION_ENABLED` in `apps/web/.env`) — controls whether the API exposes the `GET /auth/email-verified` endpoint and whether the SPA shows the "check your email" flow on signup.
...
- **WHEN** `AUTH_EMAIL_VERIFICATION_ENABLED=true` AND `[auth.email].enable_confirmations = true` in `supabase/config.toml`
...
- **WHEN** `AUTH_EMAIL_VERIFICATION_ENABLED` is unset or `false` AND `[auth.email].enable_confirmations` is `false`
...
#### Scenario: Only the API flag is on (misconfiguration)

- **WHEN** `AUTH_EMAIL_VERIFICATION_ENABLED=true` but `[auth.email].enable_confirmations` is `false` in `supabase/config.toml`
- **THEN** `GET /auth/email-verified` SHALL be reachable and the SPA SHALL route new signups to `/auth/check-email`
...
- **WHEN** `AUTH_EMAIL_VERIFICATION_ENABLED` is unset or `false` but `[auth.email].enable_confirmations = true`
- **THEN** Supabase SHALL send the confirmation email and `signUp` SHALL return no active session
- **AND** `GET /auth/email-verified` SHALL respond `404`
...
`apps/api` SHALL expose `GET /auth/email-verified` (no auth required) gated behind `AUTH_EMAIL_VERIFICATION_ENABLED`. The response SHALL be `{ verified: boolean, email: string | null }`. The endpoint SHALL accept an optional `?token=...` query parameter; when present, the server SHALL exchange the token via `supabase.auth.verifyOtp({ token, type: 'email' })` and return the resolved verification status.

#### Scenario: Verification enabled, valid token

- **WHEN** a client GETs `GET /auth/email-verified?token=<valid>` while `AUTH_EMAIL_VERIFICATION_ENABLED=true`
- **THEN** the API SHALL respond `200` with `{ verified: true, email: '<user.email>' }`

#### Scenario: Verification enabled, missing or invalid token
...
- **WHEN** `AUTH_EMAIL_VERIFICATION_ENABLED` is unset or `false`
- **THEN** `GET /auth/email-verified` SHALL respond `404 Not Found`

### Requirement: The web SPA SHALL route unverified users to a check-email page

The SPA SHALL add a `/auth/check-email` page. The registration form SHALL, on receiving a `signUp` response with no active session, redirect to `/auth/check-email` (only when `email_verification: true` on `GET /auth/features`). The check-email page SHALL poll `GET /auth/email-verified?token=<token>` if a token is present in the URL hash, otherwise instruct the user to click the link in the email.
...
- **WHEN** a user signs up while `email_verification: true` and the API returns no active session
...
- **WHEN** an unverified user navigates to a `/dashboard/*` route while `email_verification: true`
