...
Define the LinkedIn (OpenID Connect) OAuth sign-in flow on the Resumind web SPA: provider configuration at the Supabase layer, build-time feature flag, the SPA-issued PKCE redirect, callback exchange, error states, the LinkedIn Developer app enrolment steps (including the "Sign In with LinkedIn using OpenID Connect" product), and the contract with the existing Nest API Bearer-token guard.
