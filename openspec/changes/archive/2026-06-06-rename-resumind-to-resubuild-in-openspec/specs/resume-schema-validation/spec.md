...
The NestJS `ResumeSchemaValidator` SHALL load `@resubuild/schemas` (`resume.schema.json`), compile it with AJV (including format support), and reject invalid assembled resume documents before they are stored, returning 400 with structured error messages when validation fails. For normalized storage, the validator SHALL operate on the JSON Resume object produced by assembling normalized rows (or on the incoming `data` payload before disassembly on bulk import), not on raw database rows.
