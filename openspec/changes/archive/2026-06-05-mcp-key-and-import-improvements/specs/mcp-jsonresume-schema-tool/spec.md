# MCP JSON Resume schema tool (delta)

## Purpose

This delta introduces the `mcp-jsonresume-schema-tool` capability, a new
read-only MCP tool that exposes the bundled JSON Resume schema to MCP
agents so they can discover the target shape before composing a document
from non-JSON Resume source material.
