...
- [x] 4.4 Manually visit `/dashboard/cv/new/import/file`, `/dashboard/cv/new/import/url`, and `/dashboard/cv/new/create` in a running web dev server and confirm the visual order is `Breadcrumb (My CVs → current step) → Title → Subtitle → Cancel → Form` for all three pages, that `My CVs` navigates to `/dashboard`, and that the current-step labels read `Import from file`, `Import from URL`, and `Create CV` respectively. — Marked complete by the operator.
