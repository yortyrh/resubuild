...
Define dashboard CV authoring ergonomics—layout of Basics and related tabs, location controls, onboarding microcopy for sparse fields, and rich-text authoring without Markdown primacy—for `apps/web` components such as `cv-editor.tsx` and its field primitives.
...
The editor SHALL augment the following bindings with succinct helper/description lines:

- **Work**: Summary, Description, Highlights
- Education: Area, Study type
- Skills: Name, Level
- Projects: Entity, Type, Roles list, Highlights list
- Awards: Title, Awarder, Summary
- Certificates: Name, Date triad, Issuer, URL
- Publications: Name, Publisher, Release date triad, URL, Summary
- Interests: Name, Keyword list
- References: Name, Reference text

Descriptions MUST explain expected granularity (e.g., elevator pitch vs day-to-day duties vs quantifiable bullets).

#### Scenario: Work summary clarity

- **WHEN** a user focuses Work summary
...
- **WHEN** a user focuses Skill level column
- **THEN** surfaced hint SHOULD communicate meaningful scale cues (examples-based OK) without implying enum lock-in contradictory to schema
