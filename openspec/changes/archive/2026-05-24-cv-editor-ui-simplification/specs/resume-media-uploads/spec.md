...
Define authenticated, server-mediated binary upload flows powering **Basics profile photos**, leveraging Supabase Storage with a **`public.media` registry** and **API-hosted viewer URLs**. Resume JSON and Markdown embed **`GET /media/:id`** URLs (opaque UUID), not direct Storage URLs. Inline Wysimark image upload is out of scope.
