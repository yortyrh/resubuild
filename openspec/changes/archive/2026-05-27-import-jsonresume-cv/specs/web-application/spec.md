...
The new CV route SHALL ALSO expose an **Import from JSON** workflow per `cv-json-import`: file selection (and optional paste), normalization via `prepareImportedResume`, and `createCv` on explicit user confirmation. Manual create and import are separate paths on the same page; neither SHALL POST until the user confirms.
