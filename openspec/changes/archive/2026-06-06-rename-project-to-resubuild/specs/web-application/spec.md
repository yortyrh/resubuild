...
The web app's user-visible title, MCP server display name surfaced to the user, and any console log lines identifying the product SHALL use the **Resubuild** brand; the legacy "Resumind" name MUST NOT appear in client-shipped strings or browser console output.
...
#### Scenario: Client bundle ships the Resubuild brand

- **WHEN** the web app is built and served to a browser
- **THEN** the document title and any product-identification strings SHALL read "Resubuild"
- **AND** the client bundle MUST NOT contain the literal string "Resumind" (excluding `node_modules` and `openspec/changes/archive/`)

## Purpose

Capture how the Resubuild frontend authenticates users via the Nest API, calls CV endpoints with Bearer tokens, and exposes CV editing flows aligned with the API and Supabase-backed tokens (validated server-side).
