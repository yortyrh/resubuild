...
- **THEN** the SPA SHALL call `supabase.auth.signInWithOAuth({ provider: 'linkedin_oidc', options: { redirectTo: '<NEXT_PUBLIC_APP_URL or window.location.origin>/auth/callback' } })`
