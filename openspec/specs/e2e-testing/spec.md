...
- Editor library swap (cv-editor-ui change `migrate-to-mdxeditor`): web-only. All existing auth/CV/media/export/template-presentation/lifecycle/sections/AI-agent/import-LLM/import-URL/MCP E2E scenarios must continue to pass unchanged. The change replaces the Wysimark editor library with `@mdxeditor/editor`; the persisted Markdown format is unchanged, so API contract behavior is preserved.
- Responsive mobile UI + accent palette (`improve-mobile-ui` change): web-only. All existing auth/CV/media/export/template-presentation/lifecycle/sections/AI-agent/import-LLM/import-URL/MCP E2E scenarios must continue to pass unchanged. The change only adjusts mobile layouts (off-canvas section drawer below `md`, icon-only header actions below `sm`, centered auth shell, 375px dashboard header) and re-points semantic accent tokens in `globals.css`; no REST contracts, no persistence shapes, and no agent behavior change.
