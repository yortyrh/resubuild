# Auth password input visibility

## Purpose

Define the reusable web password input affordance and how the register form exposes it, so users can verify the password they typed while keeping the submitted credential byte-identical to what the auth backend already accepts.
