# API Dev Debug Surface

## Purpose

Define the local development debugging infrastructure for `apps/api`, including Node inspector endpoints, Nest Devtools integration, TypeScript source map preservation, and editor launch configuration, so agents and developers can attach debuggers and inspect the running application in non-production environments.
