...
export function composeWebEnv(input) {
  ...
  // Mirror the three auth feature flags from the previous web .env so
  // re-runs preserve operator decisions. Defaults to 'false' on the
  // first run, matching apps/web/.env.example.
...
/**
 * The three `NEXT_PUBLIC_*` mirror keys written to `apps/web/.env`.
 * `NEXT_PUBLIC_AUTH_FORGOT_PASSWORD_ENABLED` mirrors the API-side
 * `AUTH_FORGOT_PASSWORD_ENABLED` flag; the other two are web-only.
 */
...
export function runCli(target, input) {
  ...
  throw new Error(`Unknown target "${target}" — expected "api" or "web".`);
}
...
if (
  ...
  if (!target) {
    process.stderr.write('usage: node local-env-composer.mjs <api|web> [json-input]\n');
    process.exit(2);
  }
