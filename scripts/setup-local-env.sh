...
#
# Two categories of env vars are written to apps/api/.env:
#
#   1. CLI-derived (refilled every run from `supabase status -o env`):
#        SUPABASE_URL, SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY,
#        SUPABASE_JWT_SECRET, SUPABASE_PUBLISHABLE_KEY
#   2. Operator-controlled (preserved verbatim from the previous
#      apps/api/.env when present, defaulted to `false` on first run):
#        AUTH_FORGOT_PASSWORD_ENABLED
#
# The other four auth capability flags (email-verification, passwordless,
# GitHub OAuth, Google OAuth) are NOT written to the API env: they
# live exclusively on the web side as `NEXT_PUBLIC_*` mirrors and are
# resolved client-side (see apps/web/src/lib/auth/features.ts). The
# composer preserves them in apps/web/.env across re-runs.
#
# Re-running this script MUST NOT silently change operator-controlled
# flags — they encode per-environment decisions about which optional
# auth flows are exposed. Earlier versions of this script wrote a
# hardcoded heredoc that dropped these flags (and SUPABASE_PUBLISHABLE_KEY)
# every time it ran, forcing the operator to re-set them after each
# `supabase start`. The composition is now delegated to a pure Node
# module (scripts/lib/local-env-composer.mjs) so the preservation
# rule is unit-tested. See scripts/lib/local-env-composer.spec.mjs.
set -euo pipefail
...
# PUBLISHABLE_KEY was added to `supabase status -o env` in CLI ≥ 1.226
# (PR supabase/cli#4167). AuthConfigService requires it at API boot,
# so we warn loudly when the running CLI is too old to expose it —
# the composer module will fall back to whatever the previous
# apps/api/.env has, but the operator should upgrade to keep both
# files in sync.
if [[ -z "${PUBLISHABLE_KEY:-}" ]]; then
  echo "warning: supabase status did not provide PUBLISHABLE_KEY (CLI < 1.226?)." >&2
  echo "         The script will reuse the previous apps/api/.env value if present." >&2
...
# Build a JSON blob of composer inputs from shell variables. The
# composer (scripts/lib/local-env-composer.mjs) takes JSON, so the
# script's job is to marshal the shell variables into JSON without
# the shell expanding any of the values. We pipe the JSON through a
# second Node process that runs the composer.
#
# Why two Node calls instead of one inline script: keeps the
# composer in a single, testable module and avoids the shell ever
# having to quote potentially-pathological values (Supabase keys
# contain `=`, `+`, `/`, and are ~1 KB of base64 JSON).
compose_api_json() {
  API_URL="$API_URL" \
  ANON_KEY="$ANON_KEY" \
  ...
  node -e '
    const data = {
      ...
      cliPublishableKey: process.env.PUBLISHABLE_KEY,
      mediaBucket: process.env.MEDIA_BUCKET,
      mcpExportBucket: process.env.MCP_EXPORT_BUCKET,
      port: process.env.PORT,
      corsOrigin: process.env.WEB_ORIGIN,
      publicApiUrl: process.env.API_PUBLIC_URL,
      aiAgentEncryptionKey: process.env.IMPORT_LLM_CONFIG_ENCRYPTION_KEY,
      pdfImportMaxBytes: process.env.PDF_IMPORT_MAX_BYTES,
      pdfImportEnabled: process.env.PDF_IMPORT_ENABLED,
      previousEnvPath: process.env.PREVIOUS_ENV_PATH,
    };
...
compose_web_json() {
  ...
  API_URL="$API_URL" \
  PUBLISHABLE_KEY="${PUBLISHABLE_KEY:-}" \
  WEB_PREVIOUS_ENV_PATH="$WEB_ENV" \
  node -e '
    const data = {
      publicApiUrl: process.env.API_PUBLIC_URL,
      supabaseUrl: process.env.API_URL,
      cliPublishableKey: process.env.PUBLISHABLE_KEY,
      webPreviousEnvPath: process.env.WEB_PREVIOUS_ENV_PATH,
    };
    process.stdout.write(JSON.stringify(data));
  '
}
