...
export function resolveAiAgentEncryptionKey(configService: ConfigService): string | undefined {
  return (
    configService.get<string>('AI_AGENT_ENCRYPTION_KEY') ??
    configService.get<string>('IMPORT_LLM_CONFIG_ENCRYPTION_KEY')
  );
}
...
export function decryptSecret(payload: string, secret: string): string {
  ...
  try {
    ...
    throw new AiAgentDecryptionError();
  }
}
