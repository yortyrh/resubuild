...
/**
 * `resubuild://{applicationId}/application` — read one job application as JSON.
...
export class ApplicationResource {
  ...
  @ResourceTemplate({
    uriTemplate: 'resubuild://{applicationId}/application',
    name: 'Applications',
  ...
  async handle(params: { applicationId: string }) {
    ...
    return {
      contents: [
        {
          uri: `resubuild://${params.applicationId}/application`,
