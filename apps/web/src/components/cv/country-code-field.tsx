...
const ALL_OPTIONS = (Object.entries(countries) as [TCountryCode, ICountry][]).map(([code, c]) => ({
  code,
  primary: `${c.name}`,
}));
...
export function CountryCodeField({
  ...
  const selectedLabel = normalized
    ? (() => {
        const meta = ALL_OPTIONS.find((o) => o.code === normalized);
        return meta ? `${meta.primary} (${meta.code})` : normalized;
      })()
    : '';
          ...
          >
            ...
            {!trimmedQuery ? (
              <p className="text-muted-foreground mb-2 px-1 text-xs">
                Showing the first few countries — type letters to narrow the rest.
              </p>
            ) : null}
            <ul
              id={`${listId}-listbox`}
                ...
                filtered.map((item, idx) => (
                  ...
                  >
                    <button
                      type="button"
                      role="option"
                      aria-selected={normalized === item.code}
